...
Arc DevKit is a complete Python SDK for developers building on the **Arc blockchain** (by Circle) — EVM-compatible Layer 1 with USDC as gas token and Malachite consensus (<1s finality). Arc Mainnet launched 2026-09-16 and is the SDK's default network; Arc Testnet remains fully supported (`ARC_NETWORK=testnet`). Twelve modules: `Arc` facade, Dev Copilot (multi-provider: Anthropic/Gemini/OpenAI/Ollama), Payment Agent, Monitor Agent, Async Monitor, Tx Debugger, Portfolio Analyzer, USDC Token, Contracts, Event Listener, Contract Deployer, REST API + CLI.

Primary language: **Python 3.11+**. Mainnet RPC: `https://rpc.mainnet.arc.io` (chain ID `5042`). Testnet RPC: `https://rpc.testnet.arc.io` (chain ID `5042002`). Both are registered in `arc_devkit/networks.py` — see `docs/mainnet/MIGRATION_AUDIT.md` Part B for sourced network config (chain IDs, RPCs, explorers, USDC/EURC/CCTP/Gateway contract addresses). Default Claude model: `claude-sonnet-4-6` (Dev Copilot also supports Gemini/GPT/Ollama — see `COPILOT_PROVIDER`). Current version: **0.10.0**.
...
**copilot/agent.py** — `DevCopilot` is instantiated per-call (not a singleton), backed by a pluggable `LLMProvider` (`copilot/providers/`, selected via `COPILOT_PROVIDER` — `anthropic` default, or `gemini`/`openai`/`ollama`). Supports `offline=True` mode (no API calls), image attachments, multi-turn history, response cache (MD5, 5-min TTL), `ask_stream()` for SSE, and `run_agent()` (agentic tool-use loop, read-only tools only — never signs/sends). `run_agent()` raises `NotImplementedError` for any non-Anthropic provider — tool-calling schemas differ per provider and only Anthropic's is wired up (`LLMProvider.supports_tools`).

**copilot/providers/** — `LLMProvider` ABC (`base.py`: `ask()`/`ask_stream()`/`count_tokens()`/`supports_tools`, operating on a provider-neutral `ChatMessage` list) with four implementations: `anthropic_provider.py` (default, `run_agent()`-capable), `gemini_provider.py` (`pip install arc-devkit[gemini]`, google-genai — its SDK usage was checked against the installed 2.24.0 signatures but never exercised with a real API call), `openai_provider.py` (`pip install arc-devkit[openai]`), `ollama_provider.py` (local, via `httpx` directly against Ollama's `/api/chat` — no extra dependency, no API key). `get_provider(name=None)` resolves from `settings.copilot_provider`; Gemini/OpenAI/Ollama never get a guessed default model — `GEMINI_MODEL`/`OPENAI_MODEL`/`OLLAMA_MODEL` must be set explicitly or construction raises `ValueError`.
